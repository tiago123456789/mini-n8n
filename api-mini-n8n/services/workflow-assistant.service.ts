...
class WorkflowAssistantService {
    ...
    private getCreateTool() {
        return tool(
            async ({ userPrompt }: { userPrompt: string }) => {
                ...
                let prompt = "You are the assistent to build workflows based the nodes information provide below.\n"
                ...
                for (let nativeNode of nativeNodes) {
                    prompt += `${JSON.stringify({ ...nativeNode.getConfig(), isCustomNode: false })}\n\n`
                }

                for (let customNode of this.customNodes) {
                    prompt += `${JSON.stringify({ ...customNode.getConfig(), isCustomNode: true })}\n\n`
                }

                prompt += "\n\n MUST ONLY:\n"
                prompt += "- User only the nodes provided to answer the questions\n"
                prompt += "- Need to return the nodes in json format and follow the format of json from Node\n"
                prompt += "- RETURN ONLY the node data in JSON format."
                prompt += "- If found a valid node return a json ONLY, if not found return the word 'NOT_FOUND' ONLY"
                prompt += "- NO RETURN a json struct like that { node: { key_data_here: value_data_here} } ONLY RETURN a node like: { key_data_here: value_data_here }"
    ...
    private getEditNodeTool() {
        return tool(
            async ({ userPrompt, currentNodes }: { userPrompt: string, currentNodes: string }) => {

                let prompt = "You are the assistent to build and edit workflows based the nodes information provide below.\n"
                prompt += "The nodes available are:\n"
            ...
            },
            {
                ...
                schema: z.object({
                    userPrompt: z.string().describe("The message user sent"),
                    currentNodes: z.string().describe("The current nodes from workflow to edit")
                })
            }
        )
    }
    ...
    async processUserMessage(
        ...
        let prompt = "You are the assistent to help to build workflows and use the tools to answer the user instructions.\n"
        ...
        prompt += "EXTRA INFO:\n"
        prompt += "When I said 'use the output from component trigger api 2' means set value in this format here: {{this.state.steps.trigger_api_2.output}}"
        prompt += "When I said 'use the credential TOKEN or another KEY or sensitive data TOKEN or another key' means set value in this format here: {{this.state.sensitiveData.TOKENorKey}}"
