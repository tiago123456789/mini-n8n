...
function useWorkflow() {
    ...
    const triggerWorkflow = async (workflowId: string) => {
        await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/workflows/${workflowId}/trigger`);
        toast.success("Workflow triggered successfully");
    }

    const deleteWorkflow = async (workflowId: string) => {
        await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/workflows/${workflowId}`);
        getWorkflows();
    };
    ...
    const runWorkflow = async (data: {
        ...
        try {
            const response = await axios.post(
                `${process.env.NEXT_PUBLIC_API_URL}/run-workflows`, data);
    ...
    const getWorkflowById = async (id: string) => {
        const workflow = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/workflows/${id}`);
    ...
    const getCustomNodes = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/custom-nodes`);
    ...
    const createWorkflow = async (data: {
        ...
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/workflows`, {
            contextVariables: [],
            name: data.name,
            nodes: data.nodes,
    ...
    const fetchExecutions = async (workflowId: string) => {
        setIsLoading(true);
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/workflows/${workflowId}/executions`);
